...
class SmoteSampling:
    ...
    def vector_assembling(df: DataFrame, target_name: str) -> DataFrame:
        ...
        if not isinstance(df, DataFrame):
            raise TypeError("'df' must be DataFrame not " + ClassName(df))
        if not isinstance(target_name, str):
            raise TypeError("'target_name' must be str not " + ClassName(target_name))

        column_names = list(df.drop(target_name).columns)
        vec_transform = SmoteSampling.ophelia.build_vector_assembler(df, column_names)
    ...
    def split_target(df: DataFrame, field: str, minor: Any = 1, major: Any = 0) -> Dict[str, Column]:
        ...
        if not isinstance(field, str):
            raise TypeError("'field' must be str not " + ClassName(field))
    ...
    def spark_to_numpy(df: DataFrame, columns: List, spark_session: SparkSession) -> np.array:
        ...
        if not isinstance(columns, list):
            raise TypeError("'feature' must be list not " + ClassName(columns))
        if not isinstance(spark_session, SparkSession):
            raise TypeError("'spark_session' must be SparkSession not " + ClassName(spark_session))

        feature_df = SmoteSampling.ophelia.build_vector_assembler(df, columns).select("features")
        numpy_array = ParseUtils.spark_to_numpy(feature_df, spark_session)
        return numpy_array
    ...
    def numpy_to_spark(spark_session: SparkSession, feature_array, label_type=1):
        ...
        if not isinstance(feature_array, np.ndarray):
            raise TypeError("'feature_array' must be np.ndarray not " + ClassName(feature_array))
        return ParseUtils.numpy_to_vector_assembler(feature_array, label_type, spark_session)
    ...
    def __build_split_df(df, label, sample):
        return SmoteSampling.split_target(df, label)[sample]

    @staticmethod
    def __build_neighbor_list(k, algrthm, feature_mat):
        return SmoteSampling.__k_neighbor(k, algrthm, feature_mat)[1]
    ...
    def __build_synthetic_minority_over_sample(df, k, alg, pct, spark, label='label', features='features'):
        data_min = SmoteSampling.__build_split_df(df, label, "minor")
        feat_mat = SmoteSampling.spark_to_numpy(data_min, features, spark)
        neighbor = SmoteSampling.__build_neighbor_list(k, alg, feat_mat)
        min_array = SmoteSampling.__build_minor_target_array(data_min, label, spark)
        return SmoteSampling.__compute_smo(neighbor, pct, min_array)

    @staticmethod
    def __build_sample(df, smo_df, pct, label="label"):
        data_min = SmoteSampling.__build_split_df(df, label, "minor")
        data_max = SmoteSampling.__build_split_df(df, label, "major")
    ...
    def smote_sampling(spark, df, k=2, alg="auto", pct_over_min=100, pct_under_max=100):
        ...
        validate_type(alg, str, "alg")
        if alg not in ['auto', 'brute', 'kd_tree', 'ball_tree']:
            raise ValueError("unrecognized algorithm: '%s'" % alg)
        validate_type(pct_under_max, int, "pct_under_max")
        if (pct_under_max < 10) | (pct_under_max > 100):
            raise ValueError("value of variable 'pct_under_max' must be 10 <= pct <= 100")
        validate_type(pct_over_min, int, "min_pct")
        if pct_over_min < 100:
            raise ValueError("value of variable 'min_pct' must be >= 100")

        new_row = SmoteSampling.__build_synthetic_minority_over_sample(df, k, alg, pct_over_min, spark)
        smo_data_df = SmoteSampling.numpy_to_spark(spark, new_row)
        return SmoteSampling.__build_sample(df, smo_data_df, pct_under_max)
