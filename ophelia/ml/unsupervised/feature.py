...
from ophelia.ophelia import OpheliaMLException
...
class SingularVD(Transformer):
    ...
    def __to_dense_vector(self, vec_df):
        ...
        try:
            ...
            raise OpheliaMLException(f"An error occurred while calling __to_dense_vector() method: {te}")

    def __standard_features(self, df, with_mean=True, with_std=True,
        ...
        """
        try:
            vector_assemble_df = BuildVectorAssembler(df.drop(self.label_col).colmuns).transform(df)
            dense_vec_df = self.__to_dense_vector(vector_assemble_df)
            self.__logger.info("Compute Feature Standard Scaler")
            if model_path is not None:
                return BuildStandardScaler(with_mean, with_std, model_path).transform(dense_vec_df)
            return BuildStandardScaler(with_mean, with_std).transform(dense_vec_df)
        except TypeError as te:
            raise OpheliaMLException(f"An error occurred while calling __standard_features() method: {te}")
    ...
    def __index_row_matrix_rdd(self, scale_df):
        ...
        """
        try:
            vector_mllib = MLUtils.convertVectorColumnsFromML(scale_df, 'scaled_features').drop('features')
            vector_rdd = vector_mllib.select('scaled_features', 'id').rdd.map(lambda x: IndexedRow(x[1], x[0]))
            self.__logger.info("Build Index Row Matrix RDD")
            return IndexedRowMatrix(vector_rdd)
        except TypeError as te:
            raise OpheliaMLException(f"An error occurred while calling __index_row_matrix_rdd() method: {te}")
    ...
    def find_k(var_array, offset=95):
        ...
        try:
            ...
            raise OpheliaMLException(f"An error occurred while calling find_k() method: {te}")
    ...
    def svd_set_dict(self, df, d, tot_row, k, compute_u=True):
        ...
        try:
            if k is None:
                k = d
                self.__logger.warning(f"Warning: Set automatic 'K' == d == {d} since parameter is not specified")
            scaled_df = self.__standard_features(df)
            svd_rdd = self.__index_row_matrix_rdd(scaled_df).computeSVD(k=k, computeU=compute_u)
            self.__logger.warning(f"Compute SVD With K={k}, d={d}, n={tot_row}")
            return {'svd': svd_rdd, 'U': svd_rdd.U, 'S': svd_rdd.s.toArray(), 'V': svd_rdd.V, 'n': tot_row, 'd': d}
        except TypeError as te:
            raise OpheliaMLException(f"An error occurred while calling svd_set_dict() method: {te}")
    ...
    def compute_eigenvalues(self, svd):
        ...
        try:
            ...
            raise OpheliaMLException(f"An error occurred while calling compute_eigenvalues() method: {te}")
    ...
    def cumulative_variance(self, eigen_vals):
        ...
        try:
            ...
            raise OpheliaMLException(f"An error occurred while calling cumulative_variance() method: {te}")
    ...
    def __foreach_pc_var_print(self, search_func, tot_var, var_list=(75, 85, 95)):
        ...
        """
        try:
            message = (lambda var: f"Components For {var}% Variance K={search_func(tot_var, var)}")
            self.__logger.info(f"Cumulative Variance Array:\n{tot_var}")
            for v in var_list:
                self.__logger.warning(message(var=v))
        except TypeError as te:
            raise OpheliaMLException(f"An error occurred while calling __foreach_pc_var_print() method: {te}")
    ...
    def optimal_pc(self, svd_set_dict, offset):
        ...
        try:
            ...
            return PCAnalysis(optimal_k, 'data/master/ophelia/out/model/save/OptimalPCAModel')
        except TypeError as te:
            raise OpheliaMLException(f"An error occurred while calling optimal_pc() method: {te}")

    def operate(self, df):
        svd_dict = self.svd_set_dict(df=df, d=len(df.columns), tot_row=df.count(), k=self.k)
