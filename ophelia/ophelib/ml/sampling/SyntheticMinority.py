...
class SmoteSampling:
    ...
    def vector_assembling(df: DataFrame, target_name: str) -> DataFrame:
        ...
        vec_transform = MLMiner.build_vector_assembler(df, column_names)
    ...
    def split_target(df: DataFrame, field: str, minor: Any = 1, major: Any = 0) -> Dict[str, Column]:
        ...
        """
        InstanceError(df, DataFrame)
        InstanceError(field, str)
        minor_df = df[df[field] == minor]
        major_df = df[df[field] == major]
        return {"minor": minor_df, "major": major_df}

    @staticmethod
    def spark_to_numpy(df, columns, spark_session: SparkSession) -> np.array:
        """
        ...
        """
        InstanceError(df, DataFrame)
        InstanceError(columns, list)
        feature_df = MLMiner.build_vector_assembler(df, columns).select("features")
        numpy_array = DataSUtils.spark_to_numpy(feature_df, spark_session)
        return numpy_array
    ...
    def numpy_to_spark(spark_session: SparkSession, feature_array, label_type=1):
        ...
        return DataSUtils.numpy_to_vector_assembler(feature_array, label_type, spark_session)
